...
def setup_distributed(args):
    ...
    Uses tcp://{master_addr}:{master_port} with explicit world_size and rank.
    ...
    # gloo for CPU, based on assignment specs in the FAQ section :^)
    backend = "gloo"
...
def train(args, train_dataset, model, tokenizer):
    ...
    # Total batch size = per_device_batch_size * num_workers; use 16 per worker with 4 workers to match Task 1 total of 64
    total_batch = args.train_batch_size * args.gradient_accumulation_steps * (dist.get_world_size() if args.local_rank != -1 else 1)
    logger.info("  Total train batch size (w. parallel, distributed & accumulation) = %d", total_batch)
    ...
    trace_path = os.path.join(args.output_dir, "chrome_trace_ddp.json") if args.output_dir else None
...
def main():
    ...
    parser.add_argument("--per_device_train_batch_size", default=64, type=int,
                        help="Batch size per worker. Total batch = this * num_workers (e.g. 16*4=64 for 4 workers).")
    ...
    parser.add_argument("--local_rank", type=int, default=-1,
                        help="For distributed training: rank of this process (0, 1, 2, 3 for 4 nodes). Default -1 for single-node.")
    parser.add_argument("--master_addr", type=str, default="10.10.1.2",
                        help="Master node IP for distributed training (tcp init).")
    parser.add_argument("--master_port", type=str, default="12345",
                        help="Master node port for distributed training (tcp init).")
    parser.add_argument("--world_size", type=int, default=4,
                        help="Number of nodes (processes) in distributed training.")
    args = parser.parse_args()

    if os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train and not args.overwrite_output_dir:
        raise ValueError("Output directory ({}) already exists and is not empty. Use --overwrite_output_dir to overcome.".format(args.output_dir))

    # Initialize distributed process group (only needs to be invoked once)
    setup_distributed(args)

    # set up (distributed) training — CPU only
